...
#
# Логи с живого Selectel показали, что джиттера мало: конфликт держится
# дольше, чем окно из семи попыток (~1 минута), и файл терялся. Причина
# в природе 409: он снимается не мгновенно, а когда хранилище доведёт до
# конца конкурирующую операцию над контейнером. Значит лечится терпением —
# десять попыток с паузой до минуты дают окно около пяти минут, а это
# заведомо дольше самой заливки пачки, после которой бакет свободен.
# Ждущий поток почти всё это время спит, остальные три продолжают лить.
_RETRY_ATTEMPTS = 10
_RETRY_MAX_WAIT = 60
