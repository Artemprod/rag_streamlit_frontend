...
def _render_empty(focus: dict | None, query: str) -> None:
    if focus:
        ...
        st.info(
            "Для этих документов в графе знаний нет связей. Обычно это значит, "
            "что документы загрузили недавно и построение графа по ним ещё не "
            "закончилось: текстовый поиск по ним уже работает, а связи "
            "появятся позже. Вернитесь ко всему графу или поищите по названию.",
            icon="🔍",
        )
    elif query:
        st.info(
